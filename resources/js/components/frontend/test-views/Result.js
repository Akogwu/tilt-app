...
class Result extends Component {
   ...
   async componentDidMount(){
    ...
    async getResults (){
        let current_section = localStorage.getItem('@TstS3ssion');
        await  axios.get(config.apiBaseUrl+`test/result/${current_section}/summary`).then(async res => {
          if(res.data.status){
    ...
    completeResult = () => {
        ...
        //TODO add userid
        localStorage.setItem('@detailedResults','true');
        if(usrProfile == null){
            alert("You must have an account to access complete result");
            this.props.history.push("/auth/register")
        }else {
            this.props.history.push("/tests/complete-results")
        }
    };
           ...
           <main>
               ...
               <div className="text-center container mb-7">
                   ...
                   <PageHeadingButton
                       ...
                       color={"secondary"} onClick={(e) => this.completeResult(e)}/>
               </div>
