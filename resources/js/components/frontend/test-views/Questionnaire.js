...
class Questionnaire extends Component {
    ...
    async componentDidMount() {
        ...
        //localStorage.setItem("@TstS3ssion", 1);
    }
    ...
    renderGroups = () => {
        ...
        return groups.map((group, index) => {
            return (
                <Button
                    key={index}
                    variant={this.currentGroupButtonColor(group)}
                >
                    <span>
                        <i className={`fa fa-3x fa-${group.icon} pl-2 pr-2`}>
                            {" "}
                        </i>
                    </span>
    ...
    renderGroupSections = () => {
        ...
        if (!isEmpty(currentGroupSections)) {
            ...
            this.renderNoItemFoundMsg(
                "No section(s) found for the group: " +
                    !isEmpty(currentGroup.name)
                    ? currentGroup.name
                    : null
            );
        }
        ...
        if (!isEmpty(currentSectionQuestions)) {
            return currentSectionQuestions.map((questionObject, index) => (
                <QuestionItem
                    ...
                    options_id={index}
                    onAnswer={(value) =>
                        this.handleAnswer(questionObject, value)
                    }
            ...
            this.renderNoItemFoundMsg(
                "Question found for the section: " +
                    !isEmpty(currentSection.name)
                    ? currentSection.name
                    : null
            );
        }
    ...
    handleAnswer = async (questionObj, answer) => {
        await this.updateCompletedQuestions(
            questionObj.questionnaire_id,
            answer
        );
        this.setProgress();
    ...
    nextQuestionId = (questionObj) => {
        ...
        if (
            ...
            !completeAnsweredQuestions.includes(
                currentSectionQuestions[index + 1]
            )
        ) {
    ...
    submitQuestions = async (e) => {
        ...
        if (this.state.progressBarValue !== 100) {
            this.setOpentMessage(
                "error",
                "Seems you did not answer all questions."
            );
            ...
            if (!isEmpty(previousSection)) {
                ...
                this.setState(
                    (prevState) => ({
                        ...
                        currentSectionIndex:
                            groups[prevState.currentGroupIndex - 1].sections
                                .length - 1, // sets currentSectionIndex to index of last section to in previous group
                        complete: false,
                        isCurrentSectQuestComplete: isAnswered,
                    }),
                    () => {
...
if (document.getElementById("question-component")) {
    ReactDOM.render(
        <Questionnaire />,
        document.getElementById("question-component")
    );
}
